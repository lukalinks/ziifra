...
@section('title', __('settings.employee_fields.title'))
@section('header', __('settings.employee_fields.title'))

@section('content')
<div class="grid gap-8 lg:grid-cols-2">
    <div class="rounded-xl border border-ziifra-line/80 bg-ziifra-paper p-6">
        <h2 class="text-lg font-semibold text-ziifra-ink">{{ __('settings.employee_fields.add') }}</h2>
        <p class="mt-1 text-sm text-ziifra-muted">{{ __('settings.employee_fields.help') }}</p>
        <form method="POST" action="{{ route('settings.employee-fields.store') }}" class="mt-4 space-y-4" id="field-definition-form">
            @csrf
            <div>
                <label for="name" class="block text-sm font-medium text-ziifra-ink">{{ __('settings.employee_fields.field_name') }}</label>
                <input id="name" name="name" type="text" required value="{{ old('name') }}"
                    placeholder="{{ __('settings.employee_fields.name_placeholder') }}"
                    class="mt-1 block w-full rounded-lg border border-ziifra-line px-3 py-2">
                @error('name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>
            <div>
                <label for="type" class="block text-sm font-medium text-ziifra-ink">{{ __('settings.employee_fields.field_type_label') }}</label>
            ...
            <div id="options-field" class="{{ old('type') === 'select' ? '' : 'hidden' }}">
                <label for="options" class="block text-sm font-medium text-ziifra-ink">{{ __('settings.employee_fields.dropdown_options') }}</label>
                <input id="options" name="options" type="text" value="{{ old('options') }}"
                    placeholder="{{ __('settings.employee_fields.options_placeholder') }}"
                    class="mt-1 block w-full rounded-lg border border-ziifra-line px-3 py-2">
            ...
            <label class="flex items-center gap-2 text-sm text-ziifra-muted">
                ...
                {{ __('settings.employee_fields.is_required') }}
            </label>
            <button type="submit" class="rounded-lg bg-ziifra-accent px-4 py-2 text-sm font-semibold text-white hover:bg-ziifra-accent-deep">
                {{ __('settings.employee_fields.submit') }}
            </button>
        </form>
    </div>

    <div class="rounded-xl border border-ziifra-line/80 bg-ziifra-paper p-6">
        <h2 class="text-lg font-semibold text-ziifra-ink">{{ __('settings.employee_fields.defined_fields') }}</h2>
                    ...
                    <li class="flex items-center justify-between gap-4 py-3 text-sm">
                        <div>
                            ...
                            @if ($definition->is_required)
                                <span class="ml-1 text-xs text-red-600">{{ __('settings.employee_fields.required_badge') }}</span>
                            @endif
                            <p class="text-xs text-ziifra-muted">{{ trans_choice('settings.employee_fields.values_count', $definition->values_count, ['count' => $definition->values_count]) }}</p>
                        </div>
                        <form method="POST" action="{{ route('settings.employee-fields.destroy', $definition) }}"
                            data-confirm="{{ __('settings.employee_fields.confirm_remove') }}"
